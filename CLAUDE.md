...
> Switching panels changes the device tree the firmware assembles, which moves
> the DTB in memory. That is only survivable because `boot.cmd` re-derives
> `fdt_addr` every boot — see the A/B Updates section. Persisting it bricks the
> device on the next panel change, with no recovery but a reflash.
...
- **`saveenv` in `boot.cmd` must never persist `fdt_addr`, and must not write `bootdelay`/`silent`.** `saveenv` writes the *whole* environment, and U-Boot's rpi board code (`board/raspberrypi/rpi/rpi.c`, `set_fdt_addr()`) opens with `if (env_get("fdt_addr")) return;` — so a persisted `fdt_addr` freezes the DTB address at whatever the *first* boot saw and is never re-derived. The firmware anchors the DTB's end and grows it downward at 256-byte granularity, so any change to the assembled device tree moves the start address: measured on a Pi 4 B, `vc4-kms-dsi-7inch` with no panel attached gives a 70,764-byte DTB at `0x2eff1b00`, `vc4-kms-dsi-waveshare-panel,7_0_inchC,i2c1` gives 69,934 bytes at `0x2eff1e00` — 768 bytes apart. Boot with the stale address and `booti` fails `Did not find a cmdline Flattened Device Tree` on **every** boot, in both slots, forever. **Both boards are affected** — reproduced on the Pi 5 too, with the equivalent swap from its `vc4-kms-dsi-ili9881-7inch` default; it is not a Pi 4 quirk, since both use the same `boot.cmd` and the same `rpi.c` code path. `boot.cmd` therefore copies `${fdt_addr}` into `${fdtsrc}` and deletes `fdt_addr` before `saveenv`, so the firmware is asked again on every boot. Writing `bootdelay` there was the same class of bug in the other direction: it silently overwrote the release variant's `bootdelay=-2` on the first boot, undoing the non-interruptible autoboot that `post-image.sh` sets and `assert-release-image.sh` verifies at build time — a build-time assertion cannot see a value the boot script mutates at run time.
